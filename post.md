...
A RAG application is a great opportunity to address this, and even is going to come with some unexpected uses. The main goal, however, will be to automate an "expert" code commit review and summary for each week. This can remind me of what I've done, where I should go next, and how I can make better commits. 

## What is a RAG Application?
The primary goal of our RAG application will to be to provide an LLM with focused information, so that it can cite things (such as our commits) without hallucination. It will:

1. Retrieve our commits from a vector database.
2. Augment our commits by injecting them into a custom "commit review" prompt.
3. Generate a summary and review of our commits.

It's actually that simple, and most of the work is orchestrating the systems, not writing anything clever. 

## Setting Up a Vector Database With ChromaDB
A vector database will be a way to take embeddings of our commits, which are essentially numerical representations of each message. This has several advantages. Most importantly, this will allow us to perform *semantic search*. Semantic search means that we can feed in a prompt, such as "cell type prediction logic". We embed this new message, then search across the vector database for n similar commits. For example, if I search the commits for "RNA" in our analysis repo for a submitted paper (https://github.com/brocklab/clusterCleaver-analysis) focusing on biomarker separation, we find these are the top 5 commits:
...
I know it seems like a lot, but this is hugely informative. We can also tamp down the number of commits (this cost me a whole 2 cents!), or restrict them to the past week, and so on. We could also use a locally-hosted LLM in case you're using some more sensitive data.
